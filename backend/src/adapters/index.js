...
async function searchFlights(params) {   
  ...
  results.forEach((result, index) => {   
    ...
    if (result.status === 'fulfilled') {   
      logger.info(`Adapter Layer: ${source} returned ${result.value.length} flights`);   
      allFlights.push(...result.value);   
    } else {   
      logger.warn(`Adapter Layer: ${source} failed`, { error: result.reason.message });   
    }   
  });   
     
  logger.info(`Adapter Layer: Total ${allFlights.length} flights from all sources`);   
  return allFlights;   
}   
